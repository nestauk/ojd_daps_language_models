...
class CleanJobsFlow(FlowSpec):
    ...
    def get_job_ads(self):
        ...
        print("loading raw job ads")

        self.raw_job_ads = (
            download_obj(
                "open-jobs-lake",
                os.path.join(
                    "escoe_extension/outputs/data/model_application_data/",
                    "raw_job_adverts_sample.csv"
                ),
    ...
    def clean_job_ads(self):
        ...
        if self.production:
            upload_obj(
                bucket="dap-ojobert",
                path_to=os.path.join("sentence_chunks/", sent_chunk_filename),
                obj=cleaned_job_ads,
            )
        else:
            pass
