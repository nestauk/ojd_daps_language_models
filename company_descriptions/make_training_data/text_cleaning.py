...
def split_sentences(job_desc: str) -> List[str]:
    """Split a job description into sentences

    Args:
        job_desc (str): Job description to split into sentences
    ...
    tokens = nlp(job_desc)
    return [str(sent).strip() for sent in tokens.sents]
...
def clean_text(text: str) -> List[str]:
    ...
    """
    return pipe(text, detect_camelcase, replacements, split_sentences)
