...
from .data_getters import load_s3_data, load_json_dict
from .industry_getters import load_sic

# utils imports
from .bert_vectorizer import BertVectorizer
from .text_cleaning import short_hash, clean_text, split_sentences

from .sic_mapper_utils import *
...
class SicMapper(object):
    ...
    def __init__(
        ...
        self.job_id_key = "id"
        self.job_description_key = "description"
        # load company description classifier model name
        self.model_path = "nestauk/jobbert-base-cased-compdecs"
        # load relevant information to map company descriptions to SIC codes
        self.sic_comp_desc_path = "20230911_sic_company_descriptions_dict_production_True_chunksize_50.json"
        # binary of whether to use companies house data or not as part of the sic
        self.faiss_k = 100
        self.closest_distance_threshold = 0.48
        self.majority_sic_threshold = 0.3
        self.sic_levels = [2,3,4]
        self.sic_comp_desc_embeds_path = "20230911_sic_company_descriptions_embeds_production_True_chunksize_50.json"
        # load relevant information for BertVectorizer
        self.multi_process = False
        self.bert_model_name = "all-MiniLM-L6-v2"
    ...
    def load(self):
        ...
        self.hard_coded_sics = hard_coded_sics
    ...
    def preprocess_job_adverts(
        ...
        for job_advert in job_adverts:
            job_description_clean = clean_text(job_advert[self.job_description_key])
            job_description_sentences = split_sentences(job_description_clean)
    ...
    def extract_company_descriptions(
        ...
        for job_advert in preprocessed_job_adverts:
            ...
            company_description_clean = clean_company_description(
                company_description
            )
    ...
    def create_vector_index(self, sic_embeddings: np.ndarray) -> faiss.IndexFlatIP:
        ...
        """
        print(f"Creating FAISS index for vectors...")
    ...
    def predict_sic_code(self, company_embedding: np.ndarray) -> Union[str, None]:
        ...
        if closest_distance > self.closest_distance_threshold:
            ...
            if all_maj_sics != {}:
                ...
                sic_prob = calculate_average_distance(sic_code, top_sics, top_dists)
                sic_name = self.sic_names.get(sic_code)
    ...
    def get_sic_codes(self, job_adverts: Union[Dict[str, str], List[Dict[str, str]]]):
        ...
        if len(job_ids_to_predict) > 0:
            ...
            company_description_hashes = [
                short_hash(company_description)
            ...
            for job_ad in preprocessed_job_adverts_comp_desc:
                if job_ad["company_description"] != "":
                    company_desc_hash = short_hash(job_ad["company_description"])
